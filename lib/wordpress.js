...
function normalizeMenuItem(item) {
  return {
    id: item.id,
    title: displayPageTitle(item.title),
...
/*
 * The client asked for the "Founders" entry to read "Meet Us" (item 1 of the
 * Website Visual Review, 31.08.2026).
 *
 * The nav does not come from page titles: the standard /wp/v2/menus endpoint
 * is 401 anonymously, but the plugin route /merwadj/v1/menus/primary_menu
 * answers, and that is strategy 2 in getMenu(). It returns "Founders"; the
 * page itself is titled "FOUNDERS" and feeds the browser title. So the
 * rename is applied in all three getMenu strategies and in the founders
 * metadata, matched case-insensitively to cover both spellings.
 *
 * Renaming the page in WordPress does the same job and retires this entry:
 * it only ever matches the exact old title.
 */
const PAGE_TITLE_RENAMES = new Map([["founders", "Meet Us"]]);

export function displayPageTitle(title) {
  if (typeof title !== "string") return title;
  // The nav plugin says "Founders" and the page itself is titled "FOUNDERS".
  return PAGE_TITLE_RENAMES.get(title.trim().toLowerCase()) || title;
}

/*
 * The Meet Us intro is set as a bold lead line over the rest of the
 * sentence (client reference, 08.09.2026). ACF holds it as a single line,
 * so the split lives here, keyed to that exact stored value: the moment
 * anyone edits the field the key stops matching and the field's own line
 * break becomes the split instead.
 */
const INTRO_LEADS = new Map([
  [
    "MERWADJ was founded by siblings Eman and Sherif Eladawy, uniting architectural vision with engineering precision.",
    "MERWADJ was founded",
  ],
]);

export function splitIntroLead(text) {
  const value = typeof text === "string" ? text.trim() : "";
  if (!value) return { lead: "", rest: "" };

  const known = INTRO_LEADS.get(value);
  if (known) {
    return { lead: known, rest: value.slice(known.length).trim() };
  }

  const brk = value.search(/\r?\n/);
  if (brk === -1) return { lead: "", rest: value };
  return { lead: value.slice(0, brk).trim(), rest: value.slice(brk).trim() };
}

/*
 * The four symbolism icons are the same 1536x1024 screenshot of all four
 * hieroglyphs, uploaded once per icon, with each file showing its own
 * rectangle of the sheet. Two of those rectangles reach a pixel or two into
 * the neighbouring glyph, and the sliver renders as a stray white mark
 * beside the icon — the "white dots" the client reported on 31.08.2026.
 *
 * These are the same files with every glyph but their own erased. The
 * artwork is untouched: at 3x, three of the four are pixel-identical and the
 * fourth differs only in the four pixels of the mark. They also drop from
 * 198 KB each to about 40 KB.
 *
 * Keyed on the exact CMS filename, so replacing an icon in WordPress
 * retires the substitution for it.
 */
const LOCAL_ICONS = {
  "Frame-132.svg": "/icons/frame-132.svg",
  "Frame-132-1.svg": "/icons/frame-132-1.svg",
  "Frame-132-2.svg": "/icons/frame-132-2.svg",
  "Frame-132-3.svg": "/icons/frame-132-3.svg",
};

export function localIconFor(url) {
  if (typeof url !== "string") return url;
  const file = url.split("/").pop()?.split("?")[0];
  return LOCAL_ICONS[file] || url;
}
...
// The business line, in the dotted form the company uses in its signature.
const CLIENT_PHONE = "+1.913.998.0056";
      ...
      if (menu?.id) {
        ...
        if (Array.isArray(items) && items.length) {
          return items.map((item) => ({
            id: item.id,
            title: displayPageTitle(item.title?.rendered || item.title || ""),
            href: toRelativeUrl(item.url),
            target: item.target || "_self",
  ...
  try {
    ...
    if (Array.isArray(pages) && pages.length) {
      return pages
        ...
        .map((p, i) => ({
          id: p.id,
          title: displayPageTitle(p.title?.rendered || p.slug),
